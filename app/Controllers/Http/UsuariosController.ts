...
export default class UsuariosController {
  ...
  public async buscarPorNombre({ request }: HttpContextContract) {
    const nombre = request.param('nombre')
    const user = await Usuario.query().where('nombre_usuario', 'like', `%${nombre}%`).preload('perfil')
    return user;
  }
  ...
  public async actualizarUsuario({ request }: HttpContextContract) {
    ...
    const respuesta = await Usuario.query().where('codigo_usuario', id).update({
    ...
    if (respuesta[0] == 1) {
      return ({ msg: "Registro actualizado", status: 200 });
    } else {
      return ({ msg: "Registro no encontrado", status: 404 });
    }
  }

  public async eliminarUsuario({ request }: HttpContextContract) {
    const id = request.param('id');
    const respuesta = await Usuario.query().where('codigo_usuario', id).delete();
    console.log(respuesta);
    if (respuesta[0] == 1) {
      return ({ msg: "Registro eliminado", status: 200 });
    } else {
      return ({ msg: "Registro no encontrado", status: 404 });
    }
  }
